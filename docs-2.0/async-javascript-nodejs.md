...
fs.readFile('user.json', 'utf8', (err, userData) => {
    ...
    pool.query('SELECT * FROM messages WHERE name = $1', [user.name], (err, result) => {
        if (err) return handleError(err);

        result.rows.forEach((message) => {
            fs.writeFile(`message-${message.name}.txt`, message.message, (err) => {
                if (err) return handleError(err);

                logger.log(`Saved message from ${message.name}`, (err) => {
                    if (err) return handleError(err);
...
fs.readFile('user.json', 'utf8', (err, userData) => {
    ...
    pool.query('SELECT * FROM messages WHERE name = $1', [user.name], (err, result) => {
        ...
        async.map(result.rows, (message, callback) => {
            fs.writeFile(`message-${message.name}.txt`, message.message, callback);
        }, (err) => {
            if (err) return handleError(err);
...
fs.readFile('user.json', 'utf8')
    .then(userData => {
        const user = JSON.parse(userData);
        return pool.query('SELECT * FROM messages WHERE name = $1', [user.name]);
    })
    .then(result => {
        const writePromises = result.rows.map(message =>
            fs.writeFile(`message-${message.name}.txt`, message.message)
        );
...
```typescript
// From src/controllers/messageController.ts:99-135
...
```typescript
// From src/controllers/messageController.ts:273-309
...
```typescript
// From src/controllers/messageController.ts:41-83
...
```typescript
// From src/controllers/messageController.ts:324-358
...
// From src/controllers/messageController.ts:152-194
...
```typescript
// From src/controllers/messageController.ts:210-252
...
```typescript
// From src/controllers/messageController.ts:373-417
...
async function loadApplicationData() {
    ...
    const [priority1, priority2, priority3, totalCount] = await Promise.all([
        pool.query('SELECT * FROM messages WHERE priority = $1 LIMIT 10', [1]),
        pool.query('SELECT * FROM messages WHERE priority = $1 ORDER BY created_at DESC', [2]),
        pool.query('SELECT * FROM messages WHERE priority = $1', [3]),
        pool.query('SELECT COUNT(*) as total FROM messages')
    ]);

    console.timeEnd('parallel-load');  // ~100ms (one DB round-trip)

    return { priority1, priority2, priority3, totalCount };
}
...
async function loadApplicationDataSequential() {
    ...
    const priority1 = await pool.query('SELECT * FROM messages WHERE priority = $1 LIMIT 10', [1]);
    const priority2 = await pool.query('SELECT * FROM messages WHERE priority = $1 ORDER BY created_at DESC', [2]);
    const priority3 = await pool.query('SELECT * FROM messages WHERE priority = $1', [3]);
    const totalCount = await pool.query('SELECT COUNT(*) as total FROM messages');
