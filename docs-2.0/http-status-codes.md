...
Learn about HTTP status codes through practical examples and see how our TCSS-460 Message API uses these standardized response codes to communicate the results of client requests.
...
- **Practical examples** from our Message API implementation
...
**🔧 Practice:**
- Test status codes at [Swagger UI](http://localhost:4000/api-docs)
...
- Test `/src/controllers/messageController.ts` to see successful status codes
...
**🎯 Learning Note:** Our Message API doesn't use 1xx codes as they're uncommon in REST APIs.
...
| **201** | Created | Resource successfully created | `POST /message` creates something |
| **204** | No Content | Success but no content to return | `DELETE /message/:name` removes resource |
| **206** | Partial Content | Returning partial data | Paginated results or range requests |

**🔧 Try Our Examples:**
- **200 OK**: `GET /messages` - Returns list of messages
- **201 Created**: `POST /messages` - Creates a new message
...
- **400 Bad Request**: `POST /message` with invalid JSON
...
**Our Implementation:**
```typescript
// From /src/controllers/messageController.ts
export const getMessages = asyncHandler(async (request: Request, response: Response): Promise<void> => {
    const messages = await Message.findAll({
        order: [['createdAt', 'DESC']]
    });

    sendSuccess(response, messages, 'Messages retrieved successfully');
    // This sends a 200 OK by default
});
```

**🔧 Test It:** `GET /messages` - See our consistent 200 response format
...
```typescript
// From /src/controllers/messageController.ts
export const createMessage = asyncHandler(async (request: Request, response: Response): Promise<void> => {
    const { message } = request.body;
    const newMessage = await Message.create({ message });
    sendSuccess(response, newMessage, 'Message created successfully', 201);
    // Explicitly sets 201 status code
});
```

**🔧 Test It:** `POST /messages` - Notice the 201 status code for creation
...
**🔧 Test It:** Send invalid JSON to `POST /messages` to trigger 400 response
...
# Response: 201 Created with new message
```
...
**404 Not Found:**
```bash
curl http://localhost:4000/messages/99999
# Response: 404 Not Found with error details
```

**405 Method Not Allowed:**
```bash
curl -X PUT http://localhost:4000/health
...
**🔧 Explore:** Test different endpoints at [Swagger UI](http://localhost:4000/api-docs) to see this consistent format.
...
**Request:** `GET /messages/99999`
**Appropriate Response:** `404 Not Found`
**Why:** The specific message ID doesn't exist

**Our Example:** `GET /messages/99999` → 404 Not Found

### Scenario 2: User Submits Invalid Data

**Request:** `POST /messages` with empty message content
...
**Our Example:** Send malformed JSON to `POST /messages` → 400 Bad Request
...
**Request:** `DELETE /messages/123` by unauthorized user
...
2. **Try different endpoints:**
   - `GET /messages` → Expect 200 OK
   - `POST /messages` → Expect 201 Created
   - `GET /messages/99999` → Expect 404 Not Found
...
```bash
curl -i http://localhost:4000/messages
# Look for "HTTP/1.1 200 OK" in response headers

curl -i -X POST http://localhost:4000/messages -H "Content-Type: application/json" -d '{"message":"Test"}'
# Look for "HTTP/1.1 201 Created"
```

**Test error conditions:**
```bash
curl -i http://localhost:4000/messages/99999
# Look for "HTTP/1.1 404 Not Found"

curl -i -X PUT http://localhost:4000/health
...
**🔧 Immediate Practice:**
- Test all endpoints in [our API documentation](http://localhost:4000/api-docs)
...
- Review `/src/controllers/messageController.ts` for success status codes
- Examine `/src/core/utilities/responseUtils.ts` for response patterns
